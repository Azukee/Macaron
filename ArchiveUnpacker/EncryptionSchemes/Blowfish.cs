...
namespace ArchiveUnpacker.EncryptionSchemes
{
    /// <summary>
    ...
    public sealed class Blowfish : IDisposable
    {
        ...
        public string EncryptCBC(string pt)
        {
            ...
            return ByteToHex(initVector) + ByteToHex(EncryptCBC(Encoding.ASCII.GetBytes(pt)));
        }
        ...
        public string DecryptCBC(string ct)
        {
            ...
            return Encoding.ASCII.GetString(DecryptCBC(HexToByte(ct.Substring(16)))).Replace("\0", string.Empty);
        }
        ...
        {
            return ByteToHex(EncryptECB(Encoding.ASCII.GetBytes(pt)));
        }
        ...
        {
            return Encoding.ASCII.GetString(Decrypt_ECB(HexToByte(ct))).Replace("\0", string.Empty);
        }
        ...
        private void SetupKey(byte[] cipherKey)
        {
            ...
            var j = 0;
            for (var i = 0; i < 18; i++) {
                var d = (uint) (((key[j % cipherKey.Length] * 256 + key[(j + 1) % cipherKey.Length]) * 256 + key[(j + 2) % cipherKey.Length]) * 256 +
        ...
        /// </summary>
        public void Decipher(ref uint xlPar, ref uint xrPar)
        {
            xlPar ^= bfP[17];
        ...
        {
            var x1 = (bfS0[WordByte0(b)] + bfS1[WordByte1(b)]) ^ bfS2[WordByte2(b)];
            var x2 = x1 + bfS3[WordByte3(b)];
            var x3 = x2 ^ bfP[n];
            return x3 ^ a;
        }
    ...
    public sealed class BlowfishDecryptor : ICryptoTransform
    {
        ...
        public int TransformBlock(byte[] inBuffer, int offset, int count, byte[] outBuffer, int outOffset)
        {
            for (var i = 0; i < count; i += BlockSize) {
                ...
                var xr = ToUInt32(inBuffer, offset + i + 4);
                m_bf.Decipher(ref xl, ref xr);
                PlaceInArray(xl, outBuffer, outOffset + i);
                PlaceInArray(xr, outBuffer, outOffset + i + 4);
            }
        ...
        public static uint ToUInt32<TArray>(TArray value, int index) where TArray : IList<byte> => (uint) (value[index] | (value[index + 1] << 8) | (value[index + 2] << 16) | (value[index + 3] << 24));

        public static void PlaceInArray(uint value, byte[] buf, int index)
        {
